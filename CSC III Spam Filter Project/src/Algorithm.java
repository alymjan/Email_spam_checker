...
public class Algorithm {
    ...
    public void hyperLinkCheck() {//this checks the email for hyperlinks which are generally in spam emails. therefore one of our better spam checks. if the hyperlink is found it will add 4 to the score that we would use at the end of all the test's to see if it's ham or spam.
    ...
    public void triggerWords() {// this checks word for word of the email to a set list of words linked to spam emails. Once it finds one 1 will be added to the count until it meets its condition to then add 3 to the score which will be used to classify the email ham or spam.
        try {
            int count = 0;
            for (int i = 0; i < email.getWord().size(); i++) {
    ...
    public void caseSensitivity() { //this method takes each word from the EmailStorage arraylist and compares the consistency in the cases of each words. It ignores the first letter because of the English grammar and it would have takes us too long to add that too.
    ...
    public void checkNonsenseText() {// this checks for most gibberish words for example fjibdi or 89dwss08 and if it finds this in the email it will add 1 to breakout and once breakout meets its condition it will then add 2 to the score which after all the test are run will be used to calcualte ham or spam.
        int breakOut = 0;
    ...
    public void triggerPhrases() {//this compares the email to a set of phrases linked to spam emails. if it does match it will than add one to the count and as long as if the count is 1 or more 4 would be added to the score to be evaluated as ham or spam after all the test.
        try {
            int count = 0;
